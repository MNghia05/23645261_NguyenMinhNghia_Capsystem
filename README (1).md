...
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Khách hàng/Tài xế đăng ký tài khoản mới hoặc đăng nhập bằng số điện thoại, xác thực qua mã OTP. |
| **Actor chính** | Khách hàng, Tài xế |
| **Actor phụ** | Nhà cung cấp thông báo (SMS OTP) |
| **Tiền điều kiện** | Người dùng có số điện thoại hợp lệ và thiết bị nhận được SMS. |
| **Hậu điều kiện** | Tài khoản được tạo (nếu đăng ký mới) hoặc người dùng đăng nhập thành công, hệ thống cấp phiên làm việc (token). |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Người dùng nhập số điện thoại và bấm "Tiếp tục". | 2. Hệ thống kiểm tra số điện thoại đã tồn tại tài khoản hay chưa. |
| | 3. Hệ thống gửi mã OTP 6 số qua Nhà cung cấp SMS (hiệu lực 5 phút). |
| 4. Người dùng nhập mã OTP nhận được và bấm "Xác nhận". | 5. Hệ thống xác thực mã OTP. |
| | 6. Hệ thống tạo tài khoản mới (nếu chưa có) hoặc cấp token đăng nhập và chuyển vào màn hình chính. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 5.1 Người dùng nhập sai OTP. |
| | 5.1.1 Hệ thống thông báo lỗi và cho phép nhập lại (tối đa 5 lần). |
| 5.2 Người dùng bấm "Gửi lại mã" sau 60 giây. | 5.2.1 Hệ thống hủy mã cũ và gửi mã OTP mới. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 5.3 Người dùng nhập sai OTP quá 5 lần liên tiếp. |
| | 5.3.1 Hệ thống khóa tạm thời chức năng gửi OTP cho số điện thoại này trong 15 phút. |
| | 3.1 Nhà cung cấp SMS phản hồi lỗi/timeout khi gửi OTP. |
| | 3.1.1 Hệ thống thông báo "Gửi mã OTP thất bại, vui lòng thử lại" và cho phép yêu cầu gửi lại. |

---

### UC07: Đăng Ký Hồ Sơ Tài Xế (Tải Giấy Tờ)

| **Tên Use Case** | **Đăng ký hồ sơ tài xế** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng giúp Tài xế nhập thông tin cá nhân/phương tiện và tải lên giấy tờ (CCCD, Bằng lái, Đăng ký xe, Bảo hiểm) để chờ duyệt. |
| **Actor chính** | Tài xế |
| **Actor phụ** | Nhân viên vận hành |
| **Tiền điều kiện** | Tài xế đã có tài khoản (qua UC06) nhưng chưa hoàn tất hồ sơ phương tiện/giấy tờ. |
| **Hậu điều kiện** | Hồ sơ Tài xế được lưu ở trạng thái `PENDING_REVIEW`, sẵn sàng cho Nhân viên vận hành thẩm định (UC05). |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Tài xế chọn "Đăng ký làm tài xế". | 2. Hệ thống hiển thị biểu mẫu nhập thông tin cá nhân & phương tiện. |
| 3. Tài xế nhập thông tin (họ tên, biển số, loại xe) và tải ảnh CCCD, Bằng lái, Đăng ký xe, Bảo hiểm. | 4. Hệ thống kiểm tra định dạng & dung lượng ảnh hợp lệ. |
| 5. Tài xế bấm "Gửi hồ sơ". | 6. Hệ thống lưu hồ sơ ở trạng thái `PENDING_REVIEW` và gửi thông báo cho Nhân viên vận hành. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 4.1 Ảnh tải lên không đạt chuẩn (mờ, sai định dạng, quá 10MB). |
| | 4.1.1 Hệ thống từ chối và yêu cầu Tài xế tải lại ảnh đúng chuẩn. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 6.1 Mất kết nối mạng trong lúc tải ảnh lên. |
| | 6.1.1 Hệ thống lưu tạm dữ liệu đã nhập trên thiết bị và cho phép Tài xế tiếp tục khi có mạng trở lại. |

---

### UC08: Bật/Tắt Trạng Thái Sẵn Sàng Nhận Chuyến

| **Tên Use Case** | **Bật/Tắt trạng thái sẵn sàng nhận chuyến** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Tài xế chuyển đổi giữa trạng thái `AVAILABLE` (sẵn sàng nhận chuyến) và `OFFLINE`, với điều kiện ràng buộc theo BR-RULE-01. |
| **Actor chính** | Tài xế |
| **Actor phụ** | — |
| **Tiền điều kiện** | Tài xế đã đăng nhập và hồ sơ ở trạng thái `ACTIVE` (đã được duyệt). |
| **Hậu điều kiện** | Trạng thái Tài xế chuyển sang `AVAILABLE` (bắt đầu nhận tín hiệu GPS 3s/lần) hoặc `OFFLINE`. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Tài xế bật công tắc "Sẵn sàng nhận chuyến". | 2. Hệ thống kiểm tra đồng thời 3 điều kiện: hồ sơ `ACTIVE`, số dư Ví ≥ 100.000đ, Rating ≥ 3.0★. |
| | 3. Hệ thống chuyển trạng thái Tài xế sang `AVAILABLE` và bắt đầu nhận tín hiệu định vị GPS. |
| 4. Tài xế bấm tắt công tắc. | 5. Hệ thống chuyển trạng thái Tài xế sang `OFFLINE` và ngừng gửi thông báo mời chuyến. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 2.1 Một trong 3 điều kiện không thỏa mãn (ví dụ số dư ví < 100.000đ). |
| | 2.1.1 Hệ thống từ chối bật trạng thái và hiển thị lý do cụ thể (ví dụ: "Số dư ví không đủ, vui lòng nạp thêm tiền"). |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 2.2 Tài xế đang có chuyến đi dở dang (`ACCEPTED`/`ARRIVED`/`IN_PROGRESS`) và cố bấm Tắt trạng thái. |
| | 2.2.1 Hệ thống chặn thao tác và thông báo "Vui lòng hoàn tất chuyến đi hiện tại trước khi tắt trạng thái". |

---

### UC09: Khách Hàng Hủy Chuyến Đi

| **Tên Use Case** | **Khách hàng hủy chuyến đi** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Khách hàng chủ động hủy chuyến đi đã đặt, có thể phát sinh phí phạt tùy thời điểm hủy. |
| **Actor chính** | Khách hàng |
| **Actor phụ** | Tài xế |
| **Tiền điều kiện** | Chuyến đi đang ở trạng thái `PENDING` hoặc `ACCEPTED`. |
| **Hậu điều kiện** | Chuyến đi chuyển sang trạng thái `CANCELLED_BY_CUSTOMER`; có thể ghi nhận phí phạt vào tài khoản Khách hàng. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Khách hàng chọn "Hủy chuyến" trên màn hình theo dõi. | 2. Hệ thống kiểm tra thời điểm hủy so với thời điểm Tài xế nhận chuyến (`ACCEPTED`). |
| 3. Khách hàng xác nhận hủy. | 4. Hệ thống đổi trạng thái chuyến đi sang `CANCELLED_BY_CUSTOMER` và thông báo cho Tài xế (nếu đã ghép). |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 2.1 Khách hàng hủy trong vòng 2 phút kể từ khi Tài xế nhận chuyến. |
| | 2.1.1 Hệ thống không tính phí phạt. |
| | 2.2 Khách hàng hủy sau 2 phút kể từ khi Tài xế nhận chuyến. |
| | 2.2.1 Hệ thống tính phí phạt 15.000đ, ghi nợ và cộng vào cước phí chuyến đi tiếp theo của Khách hàng (BR-RULE-09). |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 4.1 Chuyến đi đã chuyển sang `ARRIVED` hoặc `IN_PROGRESS` (Tài xế đã đến/đang chở khách). |
| | 4.1.1 Hệ thống từ chối hủy qua chức năng này và yêu cầu Khách hàng liên hệ CSKH để xử lý. |

---

### UC10: Theo Dõi Hành Trình & Báo Động Khẩn Cấp (SOS)

| **Tên Use Case** | **Theo dõi hành trình & Báo động khẩn cấp (SOS)** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng hiển thị vị trí Tài xế thời gian thực cho Khách hàng và cung cấp nút báo động khẩn cấp khi có sự cố an toàn. |
| **Actor chính** | Khách hàng, Tài xế |
| **Actor phụ** | Nhân viên vận hành, Nhà cung cấp thông báo |
| **Tiền điều kiện** | Chuyến đi đang ở trạng thái `ACCEPTED` trở đi. |
| **Hậu điều kiện** | Vị trí được cập nhật liên tục trên bản đồ; nếu có SOS, sự cố được ghi nhận và chuyển tới Vận hành xử lý. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Khách hàng/Tài xế mở màn hình theo dõi chuyến đi. | 2. Hệ thống hiển thị vị trí Tài xế trên bản đồ, cập nhật mỗi 3-5 giây và tính lại ETA. |
| 3. (Tùy chọn) Người dùng bấm nút "SOS/Khẩn cấp". | 4. Hệ thống ghi nhận sự cố, gửi cảnh báo ưu tiên cao kèm tọa độ GPS tới màn hình giám sát của Nhân viên vận hành. |
| | 5. Nhân viên vận hành tiếp nhận cảnh báo và liên hệ hỗ trợ. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 2.1 Mất tín hiệu GPS quá 30 giây (BR-RULE-12). |
| | 2.1.1 Hệ thống cảnh báo Tài xế kiểm tra kết nối mạng và hiển thị trạng thái "Mất tín hiệu" cho Khách hàng. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 4.1 Không thể gửi cảnh báo tới màn hình Vận hành do lỗi hệ thống. |
| | 4.1.1 Hệ thống tự động chuyển sang phương án dự phòng: gửi SMS khẩn tới số hotline nội bộ. |

---

### UC11: Đánh Giá Tài Xế Sau Chuyến Đi

| **Tên Use Case** | **Đánh giá tài xế sau chuyến đi** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Khách hàng chấm điểm và phản hồi chất lượng dịch vụ của Tài xế sau khi hoàn tất thanh toán. |
| **Actor chính** | Khách hàng |
| **Actor phụ** | Nhân viên CSKH |
| **Tiền điều kiện** | Chuyến đi đã chuyển sang trạng thái `COMPLETED` và hoàn tất thanh toán (sau UC04). |
| **Hậu điều kiện** | Điểm rating trung bình của Tài xế được cập nhật; có thể phát sinh ticket CSKH ưu tiên cao. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| | 1. Hệ thống hiển thị popup đánh giá (1-5★) ngay sau khi hoàn tất thanh toán. |
| 2. Khách hàng chọn số sao và (tùy chọn) nhập nhận xét. | |
| 3. Khách hàng bấm "Gửi đánh giá". | 4. Hệ thống lưu đánh giá và cập nhật điểm trung bình tích lũy của Tài xế theo thời gian thực. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| 3.1 Khách hàng bấm "Bỏ qua" không đánh giá. | 3.1.1 Hệ thống đóng popup, không cập nhật rating cho chuyến này. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 2.1 Khách hàng chọn 1-2★ hoặc phản ánh về thái độ/phụ phí. |
| | 2.1.1 Hệ thống tự động khởi tạo ticket CSKH ưu tiên cao kèm nội dung phản hồi (BR-OPS-02). |
| | 4.1 Điểm rating tích lũy sau cập nhật rơi xuống dưới 3.0★ (tính trên tối thiểu 20 chuyến gần nhất). |
| | 4.1.1 Hệ thống tự động tạm khóa tài khoản Tài xế, chờ Nhân viên vận hành xử lý (BR-RULE-10). |

---

### UC12: Nạp Tiền Vào Ví Tài Xế

| **Tên Use Case** | **Nạp tiền vào Ví tài xế** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Tài xế nạp thêm tiền vào Ví điện tử để duy trì số dư tối thiểu và tiếp tục nhận chuyến. |
| **Actor chính** | Tài xế |
| **Actor phụ** | Nhà cung cấp thanh toán |
| **Tiền điều kiện** | Tài xế đã đăng nhập; số dư Ví dưới mức tối thiểu hoặc Tài xế chủ động nạp thêm. |
| **Hậu điều kiện** | Số dư Ví tài xế tăng tương ứng số tiền nạp thành công. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Tài xế chọn "Nạp tiền vào Ví" và nhập số tiền. | 2. Hệ thống chuyển hướng tới Cổng thanh toán điện tử. |
| 3. Tài xế hoàn tất thanh toán trên Cổng thanh toán. | 4. Cổng thanh toán xác nhận giao dịch thành công và trả kết quả về Hệ thống. |
| | 5. Hệ thống cộng tiền vào Ví tài xế; nếu số dư ≥ 100.000đ, tự động mở lại quyền bật trạng thái `AVAILABLE`. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| | 4.1 Giao dịch nạp tiền bị hủy giữa chừng (Tài xế thoát trang thanh toán). |
| | 4.1.1 Hệ thống giữ nguyên số dư cũ và thông báo "Giao dịch chưa hoàn tất". |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 4.2 Cổng thanh toán phản hồi lỗi hệ thống. |
| | 4.2.1 Hệ thống ghi log lỗi, hiển thị "Nạp tiền thất bại, vui lòng thử lại sau" và không cộng tiền vào Ví. |

---

### UC13: Giám Sát Chuyến Đi & Xử Lý Sự Cố (Vận Hành)

| **Tên Use Case** | **Giám sát chuyến đi & Xử lý sự cố** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng giúp Nhân viên vận hành theo dõi toàn bộ chuyến đi đang diễn ra trên Live Map Dashboard và can thiệp khi có sự cố. |
| **Actor chính** | Nhân viên vận hành |
| **Actor phụ** | Tài xế, Khách hàng |
| **Tiền điều kiện** | Nhân viên vận hành đã đăng nhập Admin Dashboard. |
| **Hậu điều kiện** | Sự cố được ghi nhận, xử lý; trạng thái chuyến đi được cập nhật nếu cần can thiệp thủ công. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Nhân viên vận hành mở "Live Map Dashboard". | 2. Hệ thống hiển thị vị trí thời gian thực của mọi chuyến đi đang diễn ra và các cảnh báo SOS (nếu có). |
| 3. Nhân viên chọn 1 chuyến đi có sự cố để xem chi tiết. | 4. Hệ thống hiển thị lịch sử trạng thái, tọa độ GPS và thông tin liên hệ Khách hàng/Tài xế. |
| 5. Nhân viên thực hiện can thiệp (gọi điện, hủy chuyến hộ, đánh dấu đã xử lý). | 6. Hệ thống ghi log thao tác can thiệp vào Audit Log. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| 5.1 Nhân viên cần hủy chuyến hộ do sự cố nghiêm trọng. | 5.1.1 Hệ thống yêu cầu nhập lý do bắt buộc trước khi đổi trạng thái chuyến sang `CANCELLED_BY_OPS`. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 2.1 Mất kết nối realtime (WebSocket) tới thiết bị Tài xế. |
| | 2.1.1 Hệ thống hiển thị biểu tượng "Mất tín hiệu" tại vị trí cuối cùng ghi nhận và lưu thời điểm mất kết nối. |

---

### UC14: Tra Cứu Lịch Sử Chuyến Đi & Giao Dịch

| **Tên Use Case** | **Tra cứu lịch sử chuyến đi & giao dịch** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng cho phép Khách hàng xem lại lịch sử chuyến đi của mình, và Nhân viên vận hành tra cứu giao dịch theo bộ lọc. |
| **Actor chính** | Khách hàng, Nhân viên vận hành |
| **Actor phụ** | — |
| **Tiền điều kiện** | Người dùng đã đăng nhập và có ít nhất một chuyến đi/giao dịch trong hệ thống. |
| **Hậu điều kiện** | Danh sách lịch sử chuyến đi/giao dịch được hiển thị theo bộ lọc yêu cầu. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Người dùng chọn "Lịch sử chuyến đi" (Khách hàng) hoặc "Tra cứu giao dịch" (Vận hành). | 2. Hệ thống truy vấn danh sách chuyến đi/giao dịch theo tài khoản hoặc bộ lọc (ngày, trạng thái, mã chuyến). |
| 3. Người dùng chọn 1 chuyến đi để xem chi tiết. | 4. Hệ thống hiển thị chi tiết: lộ trình, cước phí, phương thức thanh toán, đánh giá. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| 2.1 Người dùng áp dụng bộ lọc (khoảng ngày, trạng thái). | 2.1.1 Hệ thống trả về kết quả đã lọc tương ứng. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 2.2 Không tìm thấy dữ liệu phù hợp với bộ lọc. |
| | 2.2.1 Hệ thống hiển thị thông báo "Không có dữ liệu phù hợp". |

---

### UC15: Xem Báo Cáo Thống Kê Vận Hành (Executive Dashboard)

| **Tên Use Case** | **Xem báo cáo thống kê vận hành** |
| :--- | :--- |
| **Mô tả sơ lược** | Chức năng giúp Ban giám đốc theo dõi các chỉ số kinh doanh tổng hợp (doanh thu, tỷ lệ hoàn thành/hủy, hiệu suất) phục vụ ra quyết định chiến lược. |
| **Actor chính** | Ban giám đốc |
| **Actor phụ** | — |
| **Tiền điều kiện** | Ban giám đốc đã được cấp quyền truy cập Executive Dashboard. |
| **Hậu điều kiện** | Các chỉ số kinh doanh được hiển thị theo kỳ báo cáo đã chọn. |
| **Dòng sự kiện chính (Main Flow)** | |
| **Actor** | **System** |
| 1. Ban giám đốc đăng nhập và mở "Executive Dashboard". | 2. Hệ thống tổng hợp và hiển thị các chỉ số: tổng doanh thu, số chuyến hoàn thành/hủy, tỷ lệ ghép chuyến thành công, CSAT trung bình. |
| 3. Ban giám đốc chọn khoảng thời gian báo cáo (ngày/tuần/tháng). | 4. Hệ thống tính toán lại và cập nhật số liệu theo khoảng thời gian đã chọn. |
| **Dòng sự kiện thay thế (Alternate Flow)** | |
| 3.1 Ban giám đốc xuất báo cáo ra file (Excel/PDF). | 3.1.1 Hệ thống tạo file báo cáo và cho phép tải xuống. |
| **Dòng sự kiện ngoại lệ (Exception Flow)** | |
| | 2.1 Dữ liệu tổng hợp bị trễ do khối lượng giao dịch lớn. |
| | 2.1.1 Hệ thống hiển thị "Đang tổng hợp dữ liệu, vui lòng chờ" kèm loading indicator, ưu tiên dùng dữ liệu cache gần nhất. |
